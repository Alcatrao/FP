...
def insertionSort(lst):                        #original
...
def insertionSort(lst, key=lambda x: x):        #com função lambda para determinar o novo valor de cada elemento (para efeitos de ordenação)
    # Traverse elements starting at position 1
    if key=='len':
        key = lambda x: len(x)
    for i in range(1, len(lst)):
